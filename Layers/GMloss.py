...
class MultipleInstanceLossLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        bag_size=np.zeros(31632)
        bag_id=np.zeros(31632)
        bag_pred_prod=np.ones(31632)
	bag_true_label=np.zeros(31632)
	bag_pred_sum=np.zeros(31632)
	...
	j=0
	    ...
	    bag_pred_sum[int(i)] = bag_pred_sum[int(i)]+(pred_data[j])**(r)
	...
	for i in range(len(bag_id)):
		...
		S.append((bag_pred_sum[i]/(bag_size[i]))**(1/r))
